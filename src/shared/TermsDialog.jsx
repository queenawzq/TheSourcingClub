...
/**
 * The published document, when the live app passes one: blocks separated by
 * a blank line, the first line of each its heading, the first block the
 * title. Its headings carry their own numbers.
 */
function parsePublished(text) {
  const [title, ...sections] = String(text).split(/\n\s*\n/).map((block) => {
...
// `text` is the published document from legal_documents (app.html); the
// prototypes pass none and show the designed copy. `title` lets the same
// dialog show the privacy policy.
export function TermsDialog({ accountType = "brand", language = "en", text, title: titleOverride, onClose }) {
  ...
  const published = text ? parsePublished(text) : null;
  const title = titleOverride ?? (isChinese ? "条款与条件" : "Terms & Conditions");
  const accountLabel = titleOverride ? "The Sourcing Club" : accountType === "factory"
    ...
    <div className="terms-dialog-layer" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section className="terms-dialog" role="dialog" aria-modal="true" aria-labelledby="terms-dialog-title">
        ...
        <header>
          ...
          <p>{published ? published.title[1] : isChinese ? "生效日期：2026 年 9 月 18 日" : "Effective September 18, 2026"}</p>
        </header>
        <div className="terms-dialog-body">
          {(published?.sections ?? sectionsFor(accountType, language)).map(([heading, copy], index) => (
            <section key={heading}>
              <h3>{published ? heading : `${index + 1}. ${heading}`}</h3>
              <p>{copy}</p>
            </section>
          ))}
        </div>
